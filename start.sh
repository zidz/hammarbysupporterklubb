...
# Generates a branch-appropriate .env:
#   master (prod) -> FLASK_ENV=production, FLASK_DEBUG=0, PORT=5051
#   anything else -> FLASK_ENV=development, FLASK_DEBUG=1, PORT=5050
# A random SECRET_KEY is generated on creation.
create_env_file() {
    log_info "Checking environment file..."

    local branch flask_env flask_debug port
    branch=$(git -C "$SCRIPT_DIR" rev-parse --abbrev-ref HEAD 2>/dev/null || echo "")
    port=$(get_service_port "$SCRIPT_DIR")
...
# ===================================================
# Generated by start.sh for branch: ${branch:-unknown} (mode: ${flask_env})
#
# Read by the systemd user service (EnvironmentFile) and by the dev
# server (./start.sh --demo). Keep this file out of version control.
...
# Secret Key (auto-generated, keep private)
SECRET_KEY=${secret_key}

# Server Configuration
PORT=${port}
EOF
        log_success "Environment file created: .env (mode: ${flask_env}, port: ${port})"
    else
...
# Load .env (SECRET_KEY, FLASK_ENV, FLASK_DEBUG); explicit PORT below takes precedence
